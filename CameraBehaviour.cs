...
class CameraBehaviour : MonoBehaviour
{
    ...
    void AdjustTopDownView()
    {
        if(currentMode == Modes.FOCUSED)
        {
            ...
            relativeWorldPos.transform.position = new Vector3(transform.position.x + Mathf.Abs(player.position.x - transform.position.x), player.position.y, transform.position.z);
        }
        else
        {
